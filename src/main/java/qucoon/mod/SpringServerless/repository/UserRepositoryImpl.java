...
public class UserRepositoryImpl implements UserRepository {
    ...
    @Override
    public List<User> readByUserEmployeeId(int userEmployeeId) {
        return createQueryWithoutOnMappingFailure(environment.getDatabaseUtil().getSql2oConnection(),UserQuery.READ_BY_USER_USEREMPLOYEEID, true)
                .addParameter("userEmployeeId", userEmployeeId)
                .executeAndFetch(User.class);
    }
    @Override
    ...
    public List<User> readByUserLastLoginIpAddress(String userLastLoginIpAddress) {
        return createQueryWithoutOnMappingFailure(environment.getDatabaseUtil().getSql2oConnection(),UserQuery.READ_BY_USER_USERLASTLOGINIPADDRESS, true)
                .addParameter(System.getenv("API_KEY_USERLAST"), userLastLoginIpAddress)
                .executeAndFetch(User.class);
    }
    @Override
...
public UserPage findUser(UserFilterRequest filter) {
    ...
    if (filter.getUserEmployeeId() != null) {   baseSql.append(" AND userEmployeeId = :userEmployeeId"); }
...
if (filter.getSearch() != null) {
   baseSql.append(" & searchKeyParam & ");
     ...
     if (filter.getUserEmployeeId() != null) countQuery.addParameter("userEmployeeId", filter.getUserEmployeeId());
     ...
     if (filter.getUserLastLoginIpAddress() != null) countQuery.addParameter(System.getenv("API_KEY_USERLAST"), filter.getUserLastLoginIpAddress());
...
" OFFSET :offset ROWS FETCH NEXT :limit ROWS ONLY";
    ...
    if (filter.getUserEmployeeId() != null) pageQuery.addParameter("userEmployeeId", filter.getUserEmployeeId());
    ...
    if (filter.getUserLastLoginIpAddress() != null) pageQuery.addParameter(System.getenv("API_KEY_USERLAST"), filter.getUserLastLoginIpAddress());
    ...
    if (filter.getUserUpdatedAt() != null) pageQuery.addParameter("userUpdatedAt", filter.getUserUpdatedAt());
     ...
     return new UserPage(data, total);
       ...
       Set<String> allowedColumns = new HashSet<>(Arrays.asList(
          "userCreatedAt","userEmail","userEmployeeId","userFirstName","userId","userLastLoginDate",System.getenv("API_KEY_USERLAST"),"userLastName","userLoginCount","userPassword","userRoleId","userStatus","userUpdatedAt"
       ));
