...
@GetMapping("/read-by-user-employee-id/{userEmployeeId}")
public UserReadListResponse getUserUserEmployeeId(
@RequestHeader("Authorization") String authorization,
@PathVariable int userEmployeeId) {
JwtUtility jwt = new JwtUtility();
String className = getClass().getSimpleName();
String baseName  = className.endsWith("Controller")
        ? className.substring(0, className.length() - "Controller".length())
...
ModulePrivilege privilege = new ModulePrivilege(PrivilegeConstant.READ, moduleKey);
environment.userInfo =  jwt.privilegeAuthorization(privilege, authorization, LoginResponse.class);
    return userService.readByUserEmployeeId(userEmployeeId);
}
...
@RequestHeader("Authorization") String authorization,
        ...
        @RequestParam(required = false) Integer userEmployeeId,
    ...
    filter.setUserEmployeeId(userEmployeeId);
